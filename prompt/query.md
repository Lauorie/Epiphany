...
这篇论文中提到了一个绝妙的思想实验，完美诠释了EDL的含义。

想象两个朋友，爱丽丝和鲍勃。他们手里各有一份完全相同的“初级AI”，还商量好了同一套训练方法（比如用什么学习率、训练几轮等等）。

现在，爱丽丝拿到了一批训练数据，用这些数据训练出了一个很厉害的“高级AI”。她想把这个“高级AI”分享给鲍勃。

问题来了：怎么分享？

最笨的办法是把整个模型的参数直接发过去——但现代AI动辄有几十亿甚至上万亿参数，每个参数用32位数字表示，传输量大得吓人。

爱丽丝灵机一动，想到一个聪明的办法。

她不发模型，只发**训练数据的答案**。

具体怎么做呢？她和鲍勃约定好了一个巧妙的协议：

1. **爱丽丝用当前模型的预测，来“编码”真实答案。**

   这里的关键点是：如果模型已经能很好地预测某个答案，那这个答案就“不意外”，只需要很少的比特就能传输。但如果模型完全猜错了，那真实答案就很“意外”，需要很多比特来描述。

2. **鲍勃收到编码后，用自己的模型解码出真实答案，然后更新模型。**

3. **他们同步训练，模型始终保持一致。**

这个过程反复进行，直到所有训练数据都发送完毕。最后，鲍勃手里的模型，和爱丽丝手里的“高级AI”一模一样。

现在，关键问题来了：**爱丽丝总共发送了多少比特？**

这个总比特数，论文称之为**MDL**（最小描述长度）——它代表了训练数据里“对模型来说有多少信息”。
...
在这种情况下，问题和答案之间**没有任何规律**。AI再怎么训练，也不可能学到任何有用的东西——因为根本没有规律可学。

果然，实验结果显示：**在随机标签下，EDL 几乎为零。**

这证明了 EDL 确实在测量“可学习的结构”，而不是简单地测量“训练了多少”。你可以让 AI 拼命训练一整天，但如果数据是随机的，EDL 就是零——因为没有知识被“吸收”。
